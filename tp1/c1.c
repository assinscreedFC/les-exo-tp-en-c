...
void tri_parselection(int tab[], int taille)
{
    ...
    for (i = 0; i < taille - 1; i++)
    {
        ...
        // Échange de l'élément minimum avec l'élément à la position actuelle (i)
        if (min != i)
        {
            temporaire = tab[i];
            tab[i] = tab[min];
...
void tri_parinsertion(int tab[], int taille)
{
    ...
    for (int i = 1; i < taille; i++)
    {
        min = tab[i];
        j = i;
        // Déplacement des éléments plus grands que 'min' vers la droite
        while ((j > 0) && (tab[j - 1] > min))
        {
            tab[j] = tab[j - 1];
            j--;
        }
        // Insertion de 'min' à la position correcte
        tab[j] = min;
    }
}

// Fonction de fusion pour le tri fusion (Merge Sort)
void fusion(int tab[], int dep, int mil, int fin)
{
    ...
    // Copie des sous-tableaux gauche et droit
    for (int i = 0; i < n1; i++)
    {
        L[i] = tab[dep + i];
    }
    ...
    // Fusion des sous-tableaux gauche et droit dans le tableau principal
    while (i < n1 && j < n2)
    {
        if (L[i] <= R[j])
        {
    ...
    // Copie des éléments restants de L et R s'il en reste
    while (i < n1)
    {
        tab[k] = L[i];
        i++;
...
void tri_fusion(int tab[], int dep, int fin)
{
    if (dep < fin)
    {
        ...
        // Division en deux sous-tableaux et tri récursif
...
// Fonction de partitionnement pour le tri rapide (Quick Sort)
int partitionner(int tab[], int debut, int fin)
{
    ...
    return i; // Indice du pivot après le partitionnement
}
...
void tri_Rapide(int tab[], int debut, int fin)
{
    if (debut < fin)
    {
        ...
        // Tri récursif des deux côtés du pivot
...
void tri_abul(int tab[], int taille)
{
    ...
    for (int i = 0; i < taille - 1; i++)
    {
        for (int j = 0; j < taille - i - 1; j++)
        {
            if (tab[j] > tab[j + 1])
            {
                // Échange d'éléments adjacents si nécessaire pour trier
                temporaire = tab[j];
                tab[j] = tab[j + 1];
                tab[j + 1] = temporaire;
            }
